...
void arduinoMFCC::compute(float* frame, float* mfcc_coeffs) {
    ...
    normalize(_mfcc_coeffs, _mfcc_size);
...
void arduinoMFCC::normalize(float* vec, uint8_t length) {
    float mean = 0.0, stddev = 0.0;
    for (uint8_t i = 0; i < length; i++) mean += vec[i];
    mean /= length;
    for (uint8_t i = 0; i < length; i++) stddev += (vec[i] - mean) * (vec[i] - mean);
    stddev = sqrt(stddev / length + 1e-5); // éviter division par 0
    for (uint8_t i = 0; i < length; i++) vec[i] = (vec[i] - mean) / stddev;
}

void softmax(float in[2], float out[2]) {
  float max_val = max(in[0], in[1]);
  float sum = exp(in[0] - max_val) + exp(in[1] - max_val);
  out[0] = exp(in[0] - max_val) / sum;
  out[1] = exp(in[1] - max_val) / sum;
}
